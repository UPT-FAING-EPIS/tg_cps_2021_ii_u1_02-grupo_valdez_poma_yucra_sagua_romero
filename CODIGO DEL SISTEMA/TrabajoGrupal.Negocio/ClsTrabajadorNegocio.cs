...
namespace TrabajoGrupal.Negocio
{
    public class ClsTrabajadorNegocio
    {
        ...
        public static string Insertar(string Nombre, string Apellido, string Dni, string Rol, string Clave)
        {
            ...
            bool co;
            string Rspta="No se realizo Insercion";

            VerificarTrabajador objT = new VerificarTrabajador();
            co = objT.Verificar(Dni, Clave);

            if (co == true)
            {
                InsertarTrabajor objI = new InsertarTrabajor();
                Rspta = objI.Insertar(Nombre, Apellido, Dni, Rol, Clave);
            }
